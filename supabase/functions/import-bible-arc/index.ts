...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (count && count > 30000) {
      return new Response(
        JSON.stringify({ message: "ARC Bible already imported", count }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    // Fetch ARC Bible JSON from GitHub (thiagobodruk/biblia)
    console.log("Fetching ARC Bible from GitHub...");
    const response = await fetch(
      "https://raw.githubusercontent.com/thiagobodruk/biblia/master/json/arc.json"
    );
    
    if (!response.ok) {
      throw new Error(`Failed to fetch ARC Bible: ${response.status}`);
    }
    ...
    return new Response(
      JSON.stringify({
        message: "ARC Bible import complete",
        totalVerses,
        errors,
